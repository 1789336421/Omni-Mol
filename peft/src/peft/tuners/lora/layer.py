...
# GAL Implementation
# ====================
class LearnableAlphaLayer(nn.Module):
    def __init__(self, rank, alpha):
        super().__init__()
        self.weights = nn.Parameter(torch.tensor([alpha, 0, 0.5], dtype=torch.float), requires_grad=True)
        self.alpha = alpha
        self.rank = float(rank)
        
    def forward(self, x):
        alpha, beta, p = self.weights.unbind(0)
        x = x * (alpha.clamp(self.alpha-0.1, self.alpha+0.1) / self.rank ** p.clamp(0.5-0.01, 0.5+0.01) + beta.clamp(0-0.05, 0+0.05))
        return x

class LoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "lora_embedding_A", "lora_embedding_B", "lora_learnable_alpha")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.lora_learnable_alpha = nn.ModuleDict({})
        # For Embedding layer
    ...
    def update_layer(
        ...
        if use_learnable_alpha:
            self.lora_learnable_alpha[adapter_name] = LearnableAlphaLayer(r, lora_alpha)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            elif self.use_learnable_alpha:
                lora_output = self.lora_learnable_alpha[active_adapter](lora_B(lora_A(dropout(sub_batch))))
            else:
...
class Linear(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.use_learnable_alpha:
            alpha, beta, p = self.lora_learnable_alpha[adapter].weights.unbind(0)
            scaling = alpha.clamp(16-0.1, 16+0.1) / self.r[adapter] ** p.clamp(0.5-0.01,0.5+0.01) + beta.clamp(0-0.05, 0+0.05)
            output_tensor = transpose(weight_B @ weight_A, self.fan_in_fan_out) * scaling
        else:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    if scaling == 1:
                        ...
                        result = result + self.lora_learnable_alpha[active_adapter](lora_B(lora_A(dropout(x))))
                    else:
