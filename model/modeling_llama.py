...
def MoELlamaDecoderLayerForward(self):
    def forward(
        # self,
        ...
        **kwargs,
        ...
        # obtain moe auxilary loss
        moe_losses = []
        if not isinstance(hidden_states, torch.Tensor):
            moe_losses.append(hidden_states[1])  # get moe auxilary loss
            hidden_states = hidden_states[0]  # get moe output
...
def MoELlamaModelForward(self):
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if output_moe_loss and isinstance(layer_outputs[-1], list):
...
class CustomLlamaForCausalLM(CustomLlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # Apple loss
        # shift_embeddings = hidden_states[..., :-1, :].flatten(0, -2)
        # shift_labels = labels[..., 1:]

        # loss = linear_cross_entropy(shift_embeddings, self.lm_head.weight, shift_labels)
